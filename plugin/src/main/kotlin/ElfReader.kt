...
internal object ElfReader {
  ...
  fun maxPAlign(buf: ByteArray): Long {
    try {
      ...
      for (i in 0 until elf.phNum) {
        ...
        // Read p_type to potentially handle specific segments
        // val pType = readU32(buf, entryOffset, elf.order)
  ...
  /**
   * Checks if the ELF contains a PT_GNU_RELRO segment.
   */
  fun hasGnuRelro(buf: ByteArray): Boolean {
    try {
      val elf = parseHeader(buf) ?: return false
      var offset = elf.phOff
      for (i in 0 until elf.phNum) {
        if (offset + elf.phEntSize > buf.size) break
        val entryOffset = offset.toInt()
        val pType = readU32(buf, entryOffset, elf.order)
        if (pType == PT_GNU_RELRO) return true
        offset += elf.phEntSize
      }
    } catch (_: Throwable) {
      return false
