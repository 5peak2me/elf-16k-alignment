...
internal class ElfAlignmentPlugin : Plugin<Project> {
  ...
  override fun apply(project: Project) {
    with(project) {
      val extension =
        extensions.create("elfAlignment", ElfAlignmentExtension::class.java, project.objects)
      plugins.withType(AppPlugin::class.java) {
        ...
        androidComponents.onVariants { variant ->
          ...
          if (extension.resolveOnBuild.get()) {
            afterEvaluate {
              tasks.named("merge${variant.name.capitalizeFirstChar()}NativeLibs")
