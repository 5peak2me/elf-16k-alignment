...
internal fun ApplicationVariant.aarLibsWithJNI(): Map<String, List<File>> {
  val localLibs = this.sources.jniLibs?.all?.get()?.flatMap { directories ->
    directories.flatMap { directory ->
      directory.asFile.listFiles()?.mapNotNull { it }?.flatMap { file ->
        file.listFiles()?.mapNotNull { it }.orEmpty()
      }.orEmpty()
    }
  }.orEmpty()
  ...
  return runtimeConfiguration.incoming.artifactView {
    attributes.attribute(
      AndroidArtifacts.ARTIFACT_TYPE, // ArtifactTypeDefinition.ARTIFACT_TYPE_ATTRIBUTE
